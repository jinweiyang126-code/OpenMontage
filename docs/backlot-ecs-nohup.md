...
| 单个项目 | http://127.0.0.1:4750/p/<project-id></project> |
...
| 单个项目 | http://43.106.20.90:4750/p/<project-id></project> |
...
> **不要用宝塔「加密访问 / 密码访问」给 `/` 加密码。**  
> 面板会提示：**「目录设置加密访问后，会导致目录及子目录下的反向代理失效」**。  
> Backlot 必须走反向代理，因此密码应写在 **Nginx 反向代理的 `location` 里**（见下方步骤）。

**正确做法：在反向代理 location 内加 HTTP Basic Auth**
...
3. 在 **`proxy_pass` 同一层** 加入（与 2.2 的 SSE 配置写在一起）：
...
若已误开了「加密访问」：站点 **设置 → 加密访问 / 密码访问** 中 **删除 `/` 的规则**，再按上面改配置文件。
...
| 单个项目 | http://43.106.20.90/p/<project-id></project> |
